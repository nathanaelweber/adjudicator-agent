...
import static ch.adjudicator.agent.bitboard.model.BoardState.INDEX_KING;

public class PestoBoardEvaluation {
    ...
    static {
        int pc, p, sq;
        for (p = 0, pc = 0; p <= INDEX_KING; pc += 2, p++) {
            for (sq = 0; sq < 64; sq++) {
    ...
    public static int evaluate(BoardState boardState) {
        ...
        return (midGameScore * mgPhase + endGameScore * egPhase) / 24;
    }
}
